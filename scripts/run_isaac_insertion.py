#!/usr/bin/env python3
"""Import exported URDF assets and test inner-part insertion in Isaac Sim 6.

Run this script with Isaac Sim's ``python.sh``, not the pose_solver virtualenv.
It imports each independent URDF once, caches the resulting USD, authors a
simulation scene with a cavity-preserving static body collider, replays the
observed insertion trajectory kinematically, and then runs perturbed gravity
drop trials for the inner part.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Writable USD cache/report directory; defaults to asset-root",
    )
    parser.add_argument("--force-import", action="store_true", help="Rebuild cached USD assets")
    parser.add_argument("--skip-replay", action="store_true", help="Skip observed kinematic trajectory replay")
    parser.add_argument("--skip-drop", action="store_true", help="Skip dynamic gravity-drop trials")
...
def import_urdf_assets() -> dict[str, str]:
    ...
    if cache_path.is_file() and not ARGS.force_import:
        ...
        if cache.get("urdf_sha256") == urdf_hashes and all(Path(path).is_file() for path in cached_paths.values()):
    ...
    write_json(cache_path, {"urdf_sha256": urdf_hashes, "usd_paths": usd_paths})
    return usd_paths
...
def main() -> None:
    ...
    try:
        ...
        target_body = np.asarray(MANIFEST["assembled_T_body_from_part"][inserted], dtype=np.float64)
